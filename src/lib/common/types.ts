...
export type ParsedMeta<T> = O.Record<string, T>;
...
export type CompiledTransform = (o: O.Object) => string;
...
export type TypeDef = PrimitiveType | ClassType;

type Transform = <V, O, C>(value: unknown, object: O, context?: C) => V;
...
export type InsertMeta<TMeta extends Meta> = O.AtLeast<O.Required<TMeta>>;

export type MergedMeta<TMeta extends Meta> = Omit<TMeta, 'format'> & {
  formats: Format[];
};
...
export type TypedAjvStorageOption<TParsedResult extends ParsedResult> =
  ParseMetaOptions & {
    /**
...
export type ConstructJSONSchema<TParsedResult extends ParsedResult> = (
  schema: JSONSchemaType<AnyType>,
  parsedMeta: TParsedResult,
  source: O.Object
) => JSONSchemaType<AnyType>;

export type ConstructJTDSchema<TParsedResult extends ParsedResult> = (
  schema: ParsedJTDSchema,
  parsedMeta: TParsedResult,
  source: O.Object
) => ParsedJTDSchema;

// export type ConstructJTDType<TP>

export interface CtxValidatorFunction<T = unknown>
  extends Pick<
    ValidateFunction,
    'errors' | 'evaluated' | 'schema' | 'schemaEnv' | 'source'
  > {
