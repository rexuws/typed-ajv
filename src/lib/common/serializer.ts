import { _, Code, CodeGen, Name, str } from 'ajv/dist/ajv';
import { getCompilingSchema, SchemaEnv } from 'ajv/dist/compile';
import { and, getProperty } from 'ajv/dist/compile/codegen';
import { JTDForm, jtdForms, SchemaObjectMap } from 'ajv/dist/compile/jtd/types';
...
import type { AnySchemaObject, SchemaObject } from 'ajv/dist/types';
...
export function _compileSerializer<T>(
  this: Ajv,
  sch: SchemaEnv
): (data: T) => string {
...
function serializeSchemaProperties(
  ...
  for (const key of props) {
    serializeProperty(
      key,
      properties[key],
      keyValue(key, properties[key]?.metadata?.transform)
    );
  }
  ...
  function keyValue(
    ...
    if (transformer) {
      return gen.const(
        'value',
        _`${useFunc(gen, transformer as any)}(${data}${getProperty(
          key
        )}, ${data})`
      );
    }
